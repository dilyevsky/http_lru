...
func (c *badgerCache) writeToDB(key uint64, val *dbEntry, ttl time.Duration) error {
...
func (c *badgerCache) readFromDB(key uint64) (*dbEntry, error) {
	e := new(dbEntry)
...
func (c *badgerCache) removeFromDB(key uint64) error {
	return c.db.Update(func(txn *badger.Txn) error {
...
func (c *badgerCache) Add(key, val []byte) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	if err := c.writeToDB(h, &dbEntry{key, val}, c.ttl); err != nil {
		glog.Errorf("failed write to DB: %v", err)
		return
	}
...
// Must hold c.mu.
func (c *badgerCache) removeOldest() error {
	e := c.ll.Back()
	if e == nil {
